...
def genCompleteData(n: int = N_OBS, rng: np.random.Generator | None = None) -> pd.DataFrame:
    """
...
def imposeMar(df: pd.DataFrame, rng: np.random.Generator | None = None) -> pd.DataFrame:
    """
...
def check_correlations(n: int = 200_000, seed: int = 0) -> pd.Series:
    ...
    df = genCompleteData(n=n, rng=rng)
...
if __name__ == "__main__":
    ...
    df = genCompleteData(n=10_000, rng=rng)
    dm = imposeMar(df, rng=rng)
